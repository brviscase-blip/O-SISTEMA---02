...
export const DungeonView: React.FC<DungeonViewProps> = ({ playerStatus, setPlayerStatus, addNotification, forcedTrialWeapon, onTrialEnd }) => {
  ...
  useEffect(() => {
    if (forcedTrialWeapon) {
      const trialPortal: Portal = {
        ...
        dominantStat: String(forcedTrialWeapon.atributo_principal || 'strength').toLowerCase() as any,
        territory: forcedTrialWeapon.territorio
      };
  ...
  const executeAction = (playerAction: ActionType) => {
    ...
    let weaponAtkBonus = equippedWeapon ? equippedWeapon.dano_base : 0;
    let ignoreDefFactor = 1.0;
    let manaSteal = 0;
    
    if (equippedWeapon && equippedWeapon.desc_efeito) {
      const desc = equippedWeapon.desc_efeito.toLowerCase();
      if (desc.includes("ignora") && desc.includes("defesa")) {
        ignoreDefFactor = 0.5;
        logMsg = "[EFEITO: PENETRAÇÃO DE ARMADURA]";
      }
      if (desc.includes("recupera") && desc.includes("mana")) {
        manaSteal = 0.1;
      }
    }
    ...
    if (playerAction === enemyAction) {
      ...
      logMsg = logMsg || `CONFLITO DE FORÇAS!`;
    } else if (
      ...
      const targetDef = enemy.def * ignoreDefFactor;
      playerDmg = Math.floor(dmgPotency * 2.0) - Math.floor(targetDef);
      enemyDmg = Math.floor(enemy.atk * 0.1); 
      logMsg = logMsg || `COUNTER PERFEITO!`;
    } else {
      playerDmg = Math.floor(baseAtk * 0.3);
      enemyDmg = Math.floor(enemy.atk * 1.5);
      logMsg = logMsg || `FALHA ESTRATÉGICA!`;
    }
    ...
    setTimeout(() => {
      ...
      if (enemy.hp - playerDmg <= 0) {
        if (manaSteal > 0) {
           const manaRecovered = Math.floor(playerStatus.maxMp * manaSteal);
           setPlayerStatus(ps => ({ ...ps, mp: Math.min(ps.maxMp, ps.mp + manaRecovered) }));
           addNotification(`+${manaRecovered} MANA EXTRAÍDA`, 'success');
        }
  ...
  const calculateLoot = async () => {
    ...
    // SISTEMA DE PROBABILIDADE REAL (INCLUINDO ITENS GLOBAIS)
    const { data: items } = await client
      .from('inventario_nexus')
      .select('*')
      .or(`territorio.eq.${activePortal.territory},territorio.eq.TODOS`);

    const droppedItems: string[] = [];

    (items || []).forEach(item => {
       const roll = Math.random() * 100;
       if (roll <= Number(item.probabilidade)) {
          droppedItems.push(item.nome);
       }
    });

    if (droppedItems.length > 0) {
       addNotification(`ESPÓLIOS REIVINDICADOS: ${droppedItems.join(', ')}`, 'success');
    } else {
       addNotification("NENHUM ATIVO DETECTADO NESTA EXTRAÇÃO.", "info");
    }
    ...
    <div className="flex-1 flex flex-col bg-[#010307] text-slate-200 overflow-hidden relative h-full">
      ...
      {gameState === 'RADAR' && (
        <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-4">
            <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-2">Anomalias Geográficas Detectadas</h3>
                ...
                return (
                  <div key={p.id} onClick={() => { setActivePortal(p); setGameState('LOBBY'); }} className="cursor-pointer bg-[#030712] border-2 p-6 rounded-sm border-blue-900/40 hover:border-blue-500 transition-all group">
                    <div className="flex items-center gap-2 mb-2">
                       ...
                       <span className="text-[8px] font-black text-slate-600 uppercase italic truncate">{p.territory}</span>
                    </div>
      ...
      {gameState === 'LOBBY' && activePortal && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 bg-black/60 backdrop-blur-sm h-full">
           <div className={`max-w-md w-full bg-[#030712] border ${activePortal.enemyType === 'BOSS_TRIAL' ? 'border-amber-500/50' : 'border-blue-500/30'} p-8 rounded-sm text-center space-y-6 shadow-2xl relative overflow-hidden`}>
              <div className="absolute top-2 left-1/2 -translate-x-1/2 flex items-center gap-2">
                 <MapPin size={10} className="text-blue-500" />
                 <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest italic">{activePortal.territory}</span>
              </div>
              <div className={`w-20 h-20 mx-auto mt-4 ${activePortal.enemyType === 'BOSS_TRIAL' ? 'bg-amber-900/20 border-amber-500' : 'bg-blue-900/20 border-blue-500'} border rounded-full flex items-center justify-center ${activePortal.enemyType === 'BOSS_TRIAL' ? 'text-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.3)]' : 'text-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)]'} mb-4`}>
                <Skull size={40} />
              </div>
            ...
            <div className={`w-full max-w-xl bg-black/60 border-2 ${enemy.isTrialBoss ? 'border-amber-500/50 shadow-[0_0_20px_rgba(245,158,11,0.1)]' : 'border-slate-800'} p-4 rounded-sm flex items-center gap-6 relative`}>
              ...
              <div className="flex-1 space-y-3">
                <div className="flex justify-between items-end">
                  ...
                  <span className={`text-xs font-black ${enemy.isTrialBoss ? 'text-amber-500' : 'text-rose-500'}`}>{enemy.hp} HP</span>
                </div>
